...
pub const HOST_API: &[HostDoc] = &[
    ...
    HostDoc { sig: "host.llm.call(prompt, system)", doc: "→ Promise<string> stateless nested LLM call (seconds each). The nested model sees ONLY `prompt` + optional `system` — no contract, mounts, or host capabilities; pass whatever context it needs explicitly. There is no nested multi-turn chat. Independent calls run CONCURRENTLY under Promise.all — wall time = max, not sum" },
