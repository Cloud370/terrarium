...
会话关闭时杀死全部活进程,并等待一个有界宽限窗口(两秒)让每个 exit 被观察到;窗口内无法观察到的 exit 由 `receipts/truncated` 标记计数,审计轨迹永不静默丢失 exit。

后代不能靠脱离 anchor 的输出管道活得比 anchor 更久:spawn 的进程退出且管道关闭后,host 会清扫残余进程组(Unix);Windows 的 Job Object 在句柄关闭时做同样的事。不存在第三种生命周期。
